...
def stage1_panel_regression(rx, carry, dollar, cds=None):
    # --- long format ---
...
def stage1_panel_regression_cds(rx, carry, dollar, cds=None, base_ccy="AUD"):
    ...
    if cds is not None:
        ...
        df = df.merge(
            cds_long, on=["date", "currency"], how="inner"
        )  # inner = keep only where cds exists
