# qts-cross-asset-fx

Winter 2026 FINM 33150 Quantitative Trading Strategies — Cross-Asset FX Futures Trading Project

Research project for the FINM 33150 Quantitative Trading Strategies course, in fulfillment of the MS Financial Mathematics at the University of Chicago. The objective is to study whether lead–lag relationships between **equity and credit markets** can predict **FX futures returns**, and to evaluate tradable strategies under realistic trading assumptions.

## Researchers
- Adith Srinivasan: 12255557
- Andrew Moukabary: 12311312
- Cole Koryto: 12506473
- Jonathan Kim: 12506556
- Scott Hanna: 12496698

## Project Overview
This repository contains a research and backtesting framework for testing cross-asset predictability using equity (and credit proxies where relevant) as signals for FX futures. The strategy focuses on a small set of developed and emerging markets with varying liquidity, using standardized FX futures for implementation. Evaluation is conducted via walk-forward backtesting with transaction costs and contract rolls.

**Core questions**
- Do equity/credit signals lead FX futures in select markets?
- Are effects stronger in less liquid or higher FX-demand markets?
- Are signals robust out-of-sample and across market regimes?

## Universe (Prototype)
Initial focus on ~5–6 countries selected from Bloomberg DM/EM indexes (diverse regions; mix of liquidity levels). Final selection subject to data availability.

### Subsample Proof of Concept:
- Australia
- Brazil
- Canada
- Great Britain / United Kingdom
- Japan
- Mexico
- South Africa

## Methodology
- **Signals (X):** equity index returns, factor returns, volatility/regime features; optional credit proxies (e.g., CDS where available).
- **Targets (Y):** next-period FX futures returns (excess returns where appropriate).
- **Models:** linear baselines (ridge/lasso), with optional tree-based models.
- **Portfolio:** cross-sectional ranking on FX futures; risk controls; transaction costs; futures rolls.
- **Validation:** rolling walk-forward backtests; IC by year; regime analysis.

## Data & Tools
- **Data:** Bloomberg — Equity market indices:  
  - Mexico: MEXBOL Index (S&P/BMV IPC)  
  - Brazil: IBOV Index (Ibovespa)  
  - South Africa: JALSH Index (FTSE/JSE All Share)  
  - Japan: NKY Index (Nikkei 225)  
  - Australia: AS51 Index (S&P/ASX 200)  
  - Canada: SPTSX Index (S&P/TSX Composite)  
  - Additional data: DM/EM indexes, FX futures, selected public macro series.  
- **Stack:** Python, Jupyter, pandas/numpy, statsmodels/sklearn; plotnine/matplotlib for visualization.

### FX Spot Data

- Great Britain / United Kingdom
- Japan
- Australia
- Canada
- Mexico
- Brazil
- South Africa


#### Great Britain / United Kingdom: DEXUSUK
- File: `DEXUSUK.csv`
- Title: U.S. Dollar to British Pound Spot Exchange Rate
- Description: Daily exchange rate of U.S. Dollar (USD) to British Pound (GBP) from 1971-01-04 to 2026-02-20
- Units: U.S. Dollars to One U.K. Pound Sterling
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXUSUK](https://fred.stlouisfed.org/series/DEXUSUK)

#### Japan: DEXJPUS
- File: `DEXJPUS.csv`
- Title: Japanese Yen to U.S. Dollar Spot Exchange Rate
- Description: Daily exchange rate of Japanese Yen (JPY) to U.S. Dollar (USD) from 1971-01-04 to 2026-02-20
- Units: JPY per USD
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXJPUS](https://fred.stlouisfed.org/series/DEXJPUS)

#### Australia: DEXUSAL
- File: `DEXUSAL.csv`
- Title: U.S. Dollars to Australian Dollar Spot Exchange Rate
- Description: Daily exchange rate of U.S. Dollar (USD) to Australian Dollar (AUD) from 1971-01-04 to 2026-02-20
- Units: USD per AUD
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXUSAL](https://fred.stlouisfed.org/series/DEXUSAL)

#### Canada: DEXCAUS
- File: `DEXCAUS.csv`
- Title: Canadian Dollar to U.S. Dollar Spot Exchange Rate
- Description: Daily exchange rate of Canadian Dollar (CAD) to U.S. Dollar (USD) from 1971-01-04 to 2026-02-20
- Units: CAD per USD
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXCAUS](https://fred.stlouisfed.org/series/DEXCAUS)

#### Mexico: DEXMXUS
- File: `DEXMXUS.csv`
- Title: Mexican Pesos to U.S. Dollar Spot Exchange Rate
- Description: Daily exchange rate of Mexican Peso (MXN) to U.S. Dollar (USD) from 1993-11-08 to 2026-02-20
- Units: Mexican Pesos to One U.S. Dollar
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXMXUS](https://fred.stlouisfed.org/series/DEXMXUS)

#### Brazil: DEXBZUS
- File: `DEXBZUS.csv`
- Title: Brazilian Real to U.S. Dollar Spot Exchange Rate
- Description: Daily exchange rate of Brazilian Real (BRL) to U.S. Dollar (USD) from 1995-01-02 to 2026-02-20
- Units: Brazilian Reals to One U.S. Dollar
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXBZUS](https://fred.stlouisfed.org/series/DEXBZUS)

#### South Africa: DEXSFUS
- File: `DEXSFUS.csv`
- Title: South African Rand to U.S. Dollar Spot Exchange Rate
- Description: Daily exchange rate of South African Rand (ZAR) to U.S. Dollar (USD) from 1980-01-02 to 2026-02-20
- Units: ZAR per USD
- Source: Federal Reserve Economic Data (FRED)
- Link: [FRED DEXSFUS](https://fred.stlouisfed.org/series/DEXSFUS)


## Timeline
- **Draft due:** Friday night (for Saturday 9am submission)

## Notes / Risks
- Strict timing enforced to avoid lookahead (equity information at _t_ mapped to FX returns at _t+1_).
- FX liquidity differences are expected to attenuate signal strength in the most liquid pairs.
- Overfitting controlled via walk-forward validation and simple baselines prior to complex models.
